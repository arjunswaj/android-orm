...
class PersistenceHelper {
	...
	public PersistenceHelper(SQLiteDatabase readableDatabase,
	...
	/**
	 * Gets the ClassDetails object corresponding to the specified object. Does
	 * caching to ensure that the scanning itself is done only once.
	 * 
	 * 
	 * @param obj
	 * @return
	...
	public ClassDetails fetchClassDetailsMapping(Object obj) {
		...
		ClassDetails superClassDetails = mappingCache.get(obj.getClass()
				.getName());
	...
	public long save(Object obj, long id, Map<String, String> passedKVPs) {
		...
		if (null == inheritanceOptions) {
			...
			// Has Inheritance annotation. Awesome. Check the Strategy and
			// persist.
			id = saveObjectByInheritanceStrategy(classDetails, obj, passedKVPs);
		}
		Method setterMethod;
		try {
			// TODO: Get the type of the formal parameter instead of assuming it
			// to be long
			setterMethod = obj.getClass().getMethod(
					Utils.getSetterMethodName(Utils.getFieldTypeDetailsOfId(
							classDetails.getClassName()).getFieldName()),
					long.class);
		...
		while (null != superClassDetails) {
			...
			if (null == inheritanceOptions) {
				...
				case TABLE_PER_CLASS:
					// Get the KVPs
					populateKVPsOfTablePerClassStrategy(superClassDetails, obj,
							kvp);
					if (obj.getClass().getName()
							.equals(superClassDetails.getClassName())) {
	 ...
	 *            object to save
	 * @param id
	 *            ID of the object to be saved. -1L if needs to be ignored and
	 *            else the auto generated value
	 * @param kvp
	 *            Additional Key Value Pairs to be persisted
	 * @return
	 */
	private long saveObject(ClassDetails superClassDetails, Object obj,
			long id, Map<String, String> kvp) {
		long genId = -1;
		try {
			String tableName = (String) superClassDetails
			...
			for (FieldTypeDetails fieldTypeDetail : superClassDetails
				...
				if (null == fieldTypeDetail.getAnnotationOptionValues().get(
					...
					if (null != fieldTypeDetail.getAnnotationOptionValues()
						...
						// Don't add the id obtained from getter, it has to be
						// auto generated
						if (!columnName.equals(Constants.ID_VALUE)
							...
							Log.v(SAVE_OBJECT_TAG, "Col: " + columnName
									+ ", Val: " + columnValue);
						}
					} else if (null != fieldTypeDetail
							.getAnnotationOptionValues().get(
									Constants.ONE_TO_ONE)) {
						// Handle 1-1 Composition here
						handleOneToOneComposition(getterMethod, obj,
								fieldTypeDetail, kvp);
					}
				}
			}

			// Put the Discriminator. The column is in Super class, value is in
			// the
			// Sub class
			Map<String, Object> discriminator = superClassDetails
					.getAnnotationOptionValues().get(
							Constants.DISCRIMINATOR_COLUMN);
			if (null != discriminator
					&& !superClassDetails.getSubClassDetails().isEmpty()) {
				String discriminatorColumn = (String) discriminator
						.get(Constants.NAME);
				String discriminatorValue = (String) superClassDetails
						.getSubClassDetails().get(0)
						.getAnnotationOptionValues()
						.get(Constants.DISCRIMINATOR_VALUE)
						.get(Constants.VALUE);
				contentValues.put(discriminatorColumn, discriminatorValue);
				Log.v(SAVE_OBJECT_TAG, "DiscriminatorCol: "
						+ discriminatorColumn + ", DiscriminatorVal: "
						+ discriminatorValue);
			}
			...
			genId = insertIntoDBIfNotAlreadyPresent(id, tableName,
					contentValues);

			// Save 1-Many and other Composed Objects
			handleOneToManyAndManyToMany(superClassDetails, objClass, obj,
					genId);
		} catch (IllegalAccessException e) {
			e.printStackTrace();
		} catch (IllegalArgumentException e) {
			...
			for (FieldTypeDetails fieldTypeDetail : superClassDetails
				...
				if (null == fieldTypeDetail.getAnnotationOptionValues().get(
					...
					if (null != fieldTypeDetail.getAnnotationOptionValues()
						...
						// TODO: Will this will fail when the owning side is a
						// subclass
						...
						ClassDetails inverseSide = AnnotationsScanner
								.getInstance().getEntityObjectBranch(
										inverseClass.getName());

						// joinColumnName is different if there is a reverse
						// mapping
						...
						if (joinColumnFieldTypeDetails == null)
							...
							joinColumnName = joinColumnFieldTypeDetails
									.getFieldName();
						joinColumnName += "_"
						...
						String inverseJoinColumnName = fieldTypeDetail
								.getFieldName()
								+ "_"
						...
						String joinTableName = owningSide
								.getAnnotationOptionValues()
								.get(Constants.ENTITY).get(Constants.NAME)
								+ "_"
						...
						if (null != composedObjectCollection) {
							for (Object composedObject : composedObjectCollection) {
								...
								joinTableContentValues.put(joinColumnName,
										genId);
								joinTableContentValues.put(
										inverseJoinColumnName,
										save(composedObject,
												getId(composedObject), null));
								if (writableDatabase.insert(joinTableName,
										null, joinTableContentValues) == -1)
									Log.e(SAVE_OBJECT_TAG,
			...
			for (FieldTypeDetails fieldTypeDetail : classDetails
				...
				if (null == fieldTypeDetail.getAnnotationOptionValues().get(
					...
					if (null != fieldTypeDetail.getAnnotationOptionValues()
						...
						// Don't add the id from getter, it has to be auto
						// generated
						if (!columnName.equals(Constants.ID_VALUE)
								&& !columnName.equals(Constants.ID_VALUE_CAPS)
								&& null != columnValue) {
